...
export default function MenuPage() {
  ...
  async function openQR() {
    ...
    const dataUrl = await QRCodeLib.toDataURL(menuUrl, { width: 300, margin: 2 })
    setQrDataUrl(dataUrl)
    setQrOpen(true)
  }
  const [itemError, setItemError] = useState("")

  // Inline edit item availability
  ...
  async function saveItem() {
    ...
    setItemError("")
    setItemSaving(true)
    const isEdit = !!itemDrawer.id
    const url = isEdit ? `/api/v1/restaurant/menu/items/${itemDrawer.id}` : "/api/v1/restaurant/menu/items"
    const res = await fetch(url, {
      method: isEdit ? "PATCH" : "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
      body: JSON.stringify({
        name: itemDrawer.name,
        price: Number(itemDrawer.price),
        description: itemDrawer.description || null,
        category_id: itemDrawer.category_id || null,
        prep_time_minutes: Number(itemDrawer.prep_time_minutes) || 0,
        is_available: itemDrawer.is_available ?? true,
      }),
    })
    if (res.ok) {
      ...
      setItemError(d?.detail ?? "Failed to save.")
    }
    setItemSaving(false)
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={openQR}
            className="flex items-center gap-1.5 rounded-md border border-border px-3.5 py-2 text-sm font-medium hover:bg-muted transition-colors"
          >
            <QrCode className="h-4 w-4" />
            QR Code
          </button>
          <button
            onClick={() => setItemDrawer({ is_available: true, prep_time_minutes: 0 })}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-background rounded-xl border border-border shadow-xl p-6 space-y-4 text-center">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={() => setQrOpen(false)} className="p-1 rounded hover:bg-accent transition-colors">
                <X className="size-4" />
              </button>
            </div>
          ...
          <div className="space-y-0.5">
            <button
              ...
              className={cn(
                ...
                activeCat === "all" ? "bg-primary text-primary-foreground" : "hover:bg-accent text-foreground"
              )}
            >
              All items
              ...
              <div key={cat.id} className="flex items-center gap-1">
                <button
                  ...
                  className={cn(
                    ...
                    activeCat === cat.id ? "bg-primary text-primary-foreground" : "hover:bg-accent text-foreground"
                  )}
                >
                  {cat.name}
        ...
        <div>
          {loading ? (
            <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-3">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="h-28 rounded-lg border border-border animate-pulse bg-muted/30" />
              ))}
            </div>
      ...
      {itemDrawer !== null && (
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="absolute right-0 top-0 bottom-0 w-full max-w-md bg-background border-l border-border flex flex-col shadow-xl">
            <div className="px-5 py-4 border-b border-border font-medium">
              {itemDrawer.id ? "Edit Item" : "Add Item"}
            </div>
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
              {[
                { label: "Name *", key: "name", type: "text" },
                { label: "Price *", key: "price", type: "number" },
                { label: "Prep time (minutes)", key: "prep_time_minutes", type: "number" },
              ].map(({ label, key, type }) => (
                <div key={key} className="space-y-1.5">
                  <label className="text-sm font-medium">{label}</label>
                  <input
                    type={type}
                    value={(itemDrawer as Record<string, unknown>)[key] as string ?? ""}
              ...
              <div className="space-y-1.5">
                ...
                <select
                  value={itemDrawer.category_id ?? ""}
                  onChange={e => setItemDrawer(d => ({ ...d, category_id: e.target.value || undefined }))}
                  className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40"
                >
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  value={itemDrawer.description ?? ""}
                  onChange={e => setItemDrawer(d => ({ ...d, description: e.target.value }))}
                  rows={3}
