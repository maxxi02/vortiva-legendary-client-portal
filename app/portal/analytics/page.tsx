...
import { fetchAnalytics } from "@/lib/api/fnb"
...
export default function AnalyticsPage() {
  ...
  const { data: orders = [], isLoading } = useQuery({
    queryKey: ["analytics"],
    queryFn: () => fetchAnalytics() as Promise<Order[]>,
  })
  ...
  const topItems = useMemo(() => {
    ...
    return Object.entries(map).map(([name, v]) => ({ name, ...v })).sort((a, b) => b.revenue - a.revenue).slice(0, 10)
  }, [filtered])
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        <div className="grid grid-cols-3 gap-4">
          {[...Array(3)].map((_, i) => <div key={i} className="h-24 rounded-lg border border-border animate-pulse bg-muted/30" />)}
        </div>
      ) : (
        <>
          ...
          <div className="rounded-lg border border-border bg-card p-5 space-y-3">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={dailySales} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
                ...
                <YAxis tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} tickLine={false} axisLine={false} tickFormatter={v => `₱${(Number(v) / 1000).toFixed(0)}k`} width={48} />
                <Tooltip formatter={v => [fmt(Number(v ?? 0)), "Revenue"]} contentStyle={tooltipStyle} />
          ...
          <div className="grid lg:grid-cols-2 gap-4">
            <div className="rounded-lg border border-border bg-card p-5 space-y-3">
              ...
              {topItems.length === 0 ? <p className="text-sm text-muted-foreground py-6 text-center">No data.</p> : (
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={topItems} layout="vertical" margin={{ top: 0, right: 8, bottom: 0, left: 0 }}>
                    <XAxis type="number" tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} tickLine={false} axisLine={false} tickFormatter={v => `₱${(Number(v) / 1000).toFixed(0)}k`} />
            ...
            <div className="rounded-lg border border-border bg-card p-5 space-y-3">
              ...
              {statusBreakdown.length === 0 ? <p className="text-sm text-muted-foreground py-6 text-center">No data.</p> : (
                <ResponsiveContainer width="100%" height={220}>
